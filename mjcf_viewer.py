...
class MJCFViewer:
    ...
    def list_joints(self):
        ...
        for i in range(self.model.njnt):
            ...
            if joint_name:
                ...
                print(f"  {i+1:2d}. {joint_name:<15} 位置: {current_pos:8.3f} 速度: {current_vel:8.3f} 目标: {target:8.3f} 范围: {range_str}")
    ...
    def print_current_state(self):
        ...
        if self.model.njnt > 0:
            ...
            velocities = self.data.qvel[:self.model.njnt] if self.model.njnt <= len(self.data.qvel) else [0.0] * self.model.njnt
...
def find_mjcf_files(directory="mjcf_models/elfin3"):
    """查找可用的MJCF文件"""
    ...
    for file in os.listdir(directory):
        if file.endswith('.xml'):
            mjcf_files.append(os.path.join(directory, file))
